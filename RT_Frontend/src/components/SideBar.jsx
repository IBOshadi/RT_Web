...
if (token) {
  ...
  return (
    <div
    className={`flex ${isOpen ? "w-60" : "w-16"} bg-gradient-to-t from-[#ce521a] to-[#000000] text-white shadow-md transition-all duration-300`}
    style={{
      ...
      {(dashboardAccess === "t" || dashboardAccess === "T") && dashboardOpen && isOpen && (
       ...
       {(dashboardAccess === "t" || dashboardAccess === "T") && (
        ...
        >
          <BiSolidReport size={20} className="mr-2 ml-2 mb-4 mt-3" />
          <span className={`${isOpen ? "block" : "hidden"} ml-4`}>Report</span>
          {isOpen && (
            <FaChevronDown
              size={14}
      ...
      {(dashboardAccess === "t" || dashboardAccess === "T") && reportOpen && isOpen && (
        <ul className="ml-8 pl-8">
          <li className="flex items-center p-2 mt-4 hover:bg-[#000000]">
            <NavLink to="/1" className="w-full">
              Report 1
            </NavLink>
          </li>

          <li className="flex items-center p-2 mt-4 hover:bg-[#000000]">
            <NavLink to="/2" className="w-full">
              report 2
            </NavLink>
          </li>
